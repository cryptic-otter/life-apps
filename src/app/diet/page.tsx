  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-semibold text-gray-900">Diet</h1>
      ...
      <div className="mt-6 space-y-2">
        {entries?.map(entry => (
          <div key={entry.id} className="bg-white rounded-lg border border-gray-200 px-4 py-3 flex items-center justify-between">
            <div>
              ...
              <span className="text-sm font-medium text-gray-900">{entry.food}</span>
              {entry.calories != null && (
                <span className="text-sm text-gray-500 ml-2">{entry.calories} kcal</span>
              )}
              {entry.protein_g != null && (
                <span className="text-sm text-gray-400 ml-1">&middot; {entry.protein_g}g P</span>
              )}
              {entry.carbs_g != null && (
                <span className="text-sm text-gray-400 ml-1">&middot; {entry.carbs_g}g C</span>
              )}
              {entry.fat_g != null && (
                <span className="text-sm text-gray-400 ml-1">&middot; {entry.fat_g}g F</span>
              )}
            </div>
        ...
        {(!entries || entries.length === 0) && (
          <p className="text-sm text-gray-400 text-center py-12">No food logged today.</p>
        )}
      </div>
    </div>
  )
